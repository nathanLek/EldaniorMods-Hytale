...
/**
 * /es psell <prix> — Mettre une parcelle en vente
 */
public class ParcelSellCommand extends AbstractAsyncCommand {

    private final RequiredArg<String> priceArg;

    public ParcelSellCommand() {
        super("psell", "Mettre une parcelle en vente");
        this.priceArg = this.withRequiredArg("prix", "Prix en Or", ArgTypes.STRING);
    }
        ...
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                if (!isAdmin && !parcel.isOwner(senderUUID)) {
                    sender.getPlayerRef().sendMessage(Message.raw("Seul le proprietaire peut vendre.")); return;
                }
                ...
                sender.getPlayerRef().sendMessage(Message.raw("Parcelle mise en vente pour " + price + " Or !"));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }, world);
