...
public class TitleTwoArgCommand extends AbstractAsyncCommand {
    ...
    public CompletableFuture<Void> executeAsync(@Nonnull CommandContext ctx) {
        ...
        switch (action.toLowerCase()) {
            ...
            default -> sender.sendMessage(Message.raw("§cUsage : /es titleadmin <grant|remove|reset> <joueur> <titleId>"));
        }
    ...
    private void handleGrant(Player sender, CommandContext ctx) {
        if (!sender.hasPermission("eldanior.command.title.grant")) {
        ...
        if (title == null) {
            sender.sendMessage(Message.raw("§cErreur : Titre '" + titleId + "' inconnu."));
            sender.sendMessage(Message.raw("§7Titres disponibles : " + TitleManager.getAvailableIds()));
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                sender.sendMessage(Message.raw("§aSucces : Titre " + title.getFormattedName() + " §aaccorde a " + targetName));
    ...
    private void handleRemove(Player sender, CommandContext ctx) {
        if (!sender.hasPermission("eldanior.command.title.remove")) {
            sender.sendMessage(Message.raw("§cErreur : Pas de permission."));
            return;
        }
        ...
        if (titleId.equals("novice")) {
            sender.sendMessage(Message.raw("§cErreur : Le titre 'novice' ne peut pas etre retire."));
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                if (!data.getUnlockedTitles().contains(titleId)) {
                    sender.sendMessage(Message.raw("§cErreur : " + targetName + " ne possede pas le titre '" + titleId + "'."));
                    return;
                }
                ...
                sender.sendMessage(Message.raw("§aSucces : Titre '" + titleId + "' retire a " + targetName));
    ...
    private void handleReset(Player sender, CommandContext ctx) {
        if (!sender.hasPermission("eldanior.command.title.reset")) {
            sender.sendMessage(Message.raw("§cErreur : Pas de permission."));
            return;
        }
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                sender.sendMessage(Message.raw("§aSucces : Titres et kills de " + targetName + " reinitialises."));
