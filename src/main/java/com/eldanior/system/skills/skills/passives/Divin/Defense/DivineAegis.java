...
public class DivineAegis implements IPassiveCombatSkill {
    ...
    public void onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        float newDamage = currentDamage * 0.50f;
        damage.setAmount(newDamage);

        LOGGER.atInfo().log("[Skill] DIVINE_AEGIS activé ! Dégâts réduits de " + currentDamage + " à " + newDamage);

        if (victimRef != null) {
            PlayerRef playerRef = store.getComponent(victimRef, PlayerRef.getComponentType());
            if (playerRef != null) {
                NotificationHelper.sendNotification(playerRef, "<color:yellow>Égide Divine : -50% de dégâts</color>", NotificationStyle.Success);
            }
