...
public class CreatorVampirism implements IPassiveCombatSkill {
    @Override
    public boolean onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.70f) {
            ...
            if (manaStat != null) {
                statMap.setStatValue(StatConfig.INTELLIGENCE.getStatId(), Math.min(manaStat.getMax(), manaStat.get() + 40.0f));
            }
        }
        return false;
    }
