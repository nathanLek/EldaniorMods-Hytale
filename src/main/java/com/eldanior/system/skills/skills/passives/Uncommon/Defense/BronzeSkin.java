...
public class BronzeSkin implements IPassiveCombatSkill {
    ...
    public void onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        float newDamage = currentDamage * 0.92f;
        damage.setAmount(newDamage);

        LOGGER.atInfo().log("[Skill] BRONZE_SKIN activé ! Dégâts réduits de " + currentDamage + " à " + newDamage);

        if (victimRef != null) {
            PlayerRef playerRef = store.getComponent(victimRef, PlayerRef.getComponentType());
            if (playerRef != null) {
                NotificationHelper.sendNotification(playerRef, "<color:green>Peau de Bronze : -8% de dégâts</color>", NotificationStyle.Success);
            }
