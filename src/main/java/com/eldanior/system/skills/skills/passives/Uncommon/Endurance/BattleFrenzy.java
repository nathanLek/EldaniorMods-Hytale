...
public class BattleFrenzy implements IPassiveCombatSkill {
    ...
    public boolean onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        // 30% de chance de restaurer 8 endurance en attaquant
        if (Math.random() <= 0.30f) {
            ...
            if (enduranceStat != null) {
                float newEndurance = Math.min(enduranceStat.getMax(), enduranceStat.get() + 8.0f);
