...
public class AdamantineLungs implements IPassiveCombatSkill {
    @Override
    public boolean onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.35f) {
            ...
            if (enduranceStat != null) {
                statMap.setStatValue(StatConfig.ENDURANCE.getStatId(), Math.min(enduranceStat.getMax(), enduranceStat.get() + 16.0f));
            }
        }
        return false;
    }
