...
public class ArcaneVampirism implements IPassiveCombatSkill {
    @Override
    public boolean onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.35f) {
            ...
            if (manaStat != null) {
                statMap.setStatValue(StatConfig.INTELLIGENCE.getStatId(), Math.min(manaStat.getMax(), manaStat.get() + 16.0f));
            }
        }
        return false;
    }
