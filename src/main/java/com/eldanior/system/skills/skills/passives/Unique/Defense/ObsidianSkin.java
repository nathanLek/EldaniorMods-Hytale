...
public class ObsidianSkin implements IPassiveCombatSkill {
    ...
    public void onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        float newDamage = currentDamage * 0.75f;
        damage.setAmount(newDamage);

        LOGGER.atInfo().log("[Skill] OBSIDIAN_SKIN activé ! Dégâts réduits de " + currentDamage + " à " + newDamage);

        if (victimRef != null) {
            PlayerRef playerRef = store.getComponent(victimRef, PlayerRef.getComponentType());
            if (playerRef != null) {
                NotificationHelper.sendNotification(playerRef, "<color:red>Peau d'Obsidienne : -25% de dégâts</color>", NotificationStyle.Success);
            }
