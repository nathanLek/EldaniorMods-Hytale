...
public class CombatIntuition implements IPassiveCombatSkill {
    @Override
    public float getStatMultiplier(StatConfig stat) {
        if (stat == StatConfig.THREAT_AWARENESS) return 2.20f;
        return 1.0f;
    }
    @Override
    public String getRadarMessage(List<RadarTarget> closestTargets, int extraMobs, int extraPlayers) {
        ...
        return "<color:red>" + total + " menace(s) identifiée(s)</color>";
    }
    @Override
