...
public class PhoenixBlood implements IPassiveCombatSkill {
    @Override
    public boolean onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.30f) {
            ...
            if (healthStat != null) {
                statMap.setStatValue(StatConfig.VITALITY.getStatId(), Math.min(healthStat.getMax(), healthStat.get() + healthStat.getMax() * 0.16f));
            }
        }
        return false;
    }
