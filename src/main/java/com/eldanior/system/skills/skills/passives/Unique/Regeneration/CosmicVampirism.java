...
public class CosmicVampirism implements IPassiveCombatSkill {
    @Override
    public boolean onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.40f) {
            ...
            if (manaStat != null) {
                statMap.setStatValue(StatConfig.INTELLIGENCE.getStatId(), Math.min(manaStat.getMax(), manaStat.get() + 22.0f));
            }
        }
        return false;
    }
