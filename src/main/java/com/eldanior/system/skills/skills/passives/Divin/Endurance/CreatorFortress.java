package com.eldanior.system.skills.skills.passives.Divin.Endurance;
...
public class CreatorFortress implements IPassiveCombatSkill {
    @Override
    public void onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (enduranceStat != null && enduranceStat.get() >= (enduranceStat.getMax() * 0.20f)) {
            damage.setAmount(damage.getAmount() * 0.60f);
        }
    }
}