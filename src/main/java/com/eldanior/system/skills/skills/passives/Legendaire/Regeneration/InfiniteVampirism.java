...
public class InfiniteVampirism implements IPassiveCombatSkill {
    @Override
    public void onAttack(Damage damage, PlayerLevelData attackerData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        if (Math.random() <= 0.45f) {
            ...
            if (manaStat != null) {
                statMap.setStatValue(StatConfig.INTELLIGENCE.getStatId(), Math.min(manaStat.getMax(), manaStat.get() + 30.0f));
            }
        }
    }
}