...
public class SteelSkin implements IPassiveCombatSkill {
    ...
    public void onDefend(Damage damage, PlayerLevelData victimData, Store<EntityStore> store, Ref<EntityStore> attackerRef, Ref<EntityStore> victimRef) {
        ...
        float newDamage = currentDamage * 0.85f;
        damage.setAmount(newDamage);

        LOGGER.atInfo().log("[Skill] STEEL_SKIN activé ! Dégâts réduits de " + currentDamage + " à " + newDamage);

        if (victimRef != null) {
            PlayerRef playerRef = store.getComponent(victimRef, PlayerRef.getComponentType());
            if (playerRef != null) {
                NotificationHelper.sendNotification(playerRef, "<color:light_purple>Peau d'Acier : -15% de dégâts</color>", NotificationStyle.Success);
            }
